<?php 
    ...
    if(isset($_POST["exportarSospecha"])) {
        if(!empty($consulta3)){
            $ficheroExcel="DEIT_PASCO CG_FT_GESTANTES CON SOSPECHA DE VIOLENCIA "._date("d-m-Y", false, 'America/Lima').".xls";        
...
<?php
    ...
    if(isset($_POST["exportarTratamiento"])) {
        if(!empty($consulta8)){
            $ficheroExcel="DEIT_PASCO CG_FT_GESTANTES CON INICIO DE TRATAMIENTO "._date("d-m-Y", false, 'America/Lima').".xls";        
            header('Content-Type: application/vnd.ms-excel');
            header("Content-Type: application/octet-stream");
            header("Content-Disposition: attachment;filename=".$ficheroExcel);
            header("Cache-Control: max-age=0");
?>
    ...
    <table>
        <thead>
            ...
            <tr></tr>
            <tr class="text-center">
                <th colspan="11" style="font-size: 26px; border: 1px solid #3A3838;">Gestantes con Inicio de Tratamiento - <?php echo $nombre_mes; ?></th>
            </tr>
            <tr></tr>
            <tr>
