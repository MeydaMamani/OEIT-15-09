  ...
  <br>
    ...
    <?php
        ...
        if ($dist_1 == 'TODOS') {
            ...
            $resultado = "SELECT A.Provincia_Establecimiento, A.Distrito_Establecimiento, A.Nombre_Establecimiento, A.Abrev_Tipo_Doc_Paciente, A.Numero_Documento_Paciente,
                            ...
                            Min(CASE WHEN (anio='2021' and Tipo_Diagnostico='D' and Codigo_Item in ('99199.28','Z292') AND (Valor_Lab IS NULL OR Valor_Lab > '2'))THEN A.Fecha_Atencion ELSE NULL END)'EN_BLANCO'
                                ...
                                on a.Numero_Documento_Paciente=m.Numero_Documento_Paciente WHERE
                            ...
                            GROUP BY A.Provincia_Establecimiento, A.Abrev_Tipo_Doc_Paciente, A.Edad_Reg, m.apellido_paterno_paciente, m.Apellido_Materno_Paciente,
                            m.Nombres_Paciente, A.Numero_Documento_Paciente, A.Distrito_Establecimiento, A.Nombre_Establecimiento,a.Grupo_Edad, A.Fecha_Nacimiento_Paciente
        ...
        <div class="col-11 mb-4 text-end">
            <button type="submit" name="Limpiar" class="btn btn-secondary 1btn_buscar" onclick="location.href='desparacitacion';"><i class="fa fa-arrow-left"></i> Regresar</button>
        </div>
        <div class="row">
            <div class="col-1"></div>
                ...
                <table class="table table-hover table-bordered text-center table-responsive">
                    <thead class="text-light" style="background: #538fbc;">
                        <tr>
                            ...
                            <th class="align-middle">NOMBRE PACIENTE</th>
                            <th class="align-middle">GRUPO EDAD</th>
                            <th class="align-middle">EDAD REGISTRO</th> 
                            <th class="align-middle">FECHA NACIMIENTO</th>
                            <th class="align-middle">PRIMERA</th>
                            <th class="align-middle">SEGUNDA</th>
                            <th class="align-middle">EN BLANCO</th>
                        </tr>
                    </thead>
                    <?php  
                    $i=1;
                    while ($consulta = sqlsrv_fetch_array($consulta2)){  
                        ...
                        if(is_null ($consulta['apellido_paterno_paciente']) ){
                            $newdate8 = '  -'; }
                            else{
                        $newdate8 = $consulta['apellido_paterno_paciente'];}

                        if(is_null ($consulta['Apellido_Materno_Paciente']) ){
                            $newdate9 = '  -'; }
                            else{
                        $newdate9 = $consulta['Apellido_Materno_Paciente'];}

                        if(is_null ($consulta['Nombres_Paciente']) ){
                            $newdate10 = '  -'; }
                            else{
                        $newdate10 = $consulta['Nombres_Paciente'];}

                        if(is_null ($consulta['Grupo_Edad']) ){
                            $newdate11 = '  -'; }
                            else{
                        $newdate11 = $consulta['Grupo_Edad'];}

                        if(is_null ($consulta['Edad_Reg']) ){
                            $newdate12 = '  -'; }
                            else{
                        $newdate12 = $consulta['Edad_Reg'];}
            
                        if(is_null ($consulta['Fecha_Nacimiento_Paciente']) ){
                            $newdate13 = '  -'; }
                            else{
                        $newdate13 = $consulta['Fecha_Nacimiento_Paciente'] -> format('d/m/y');}
           
                        if(is_null ($consulta['PRIMERA']) ){
                            $newdate14 = '  -'; }
                            else{
                        $newdate14 = $consulta['PRIMERA'] -> format('d/m/y');}
            
                        if(is_null ($consulta['SEGUNDA']) ){
                            $newdate15 = '  -'; }
                            else{ 
                            $newdate15 = $consulta['SEGUNDA']-> format('d/m/y');}
                                    
                        if(is_null ($consulta['EN_BLANCO']) ){
                            $newdate16 = '  -'; }
                            else{
                            $newdate16 = $consulta['EN_BLANCO'] -> format('d/m/y');}
